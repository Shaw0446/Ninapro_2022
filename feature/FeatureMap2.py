...
def two_dimension_graph(feature):
    feature = feature.reshape((feature.shape[0], -1, channal))
...
for j in range(1, 2):
    file = h5py.File(dir+'/data/down_Fea/DB2_s' + str(j) + 'fea.h5', 'r')
    '''step1: 获取特征数组'''
    ...
    file = h5py.File(dir+'/data/featureMap/DB2_s' + str(j) + 'map.h5', 'w')
