...
def dataCombin(data, label, rep_arr, reps):
    if reps:
        ...
        target_data = data[indices, :]
        target_label = label[indices]
        target_rep = rep_arr[indices]

    return target_data, target_label, target_rep
...
if __name__ == '__main__':
    for j in range(1, 2):
        ...
        '''将stimulus和rep重新拼接成df格式便于存放和提取'''
        df1 = pd.DataFrame()


        '''存储为h5文件 '''
        # df1.to_hdf(dir + '/data/Comb_Seg/DB2_s' + str(j) + 'Seg.h5', format='table', key='df', mode='w',
        ...
        file = h5py.File(dir+'/data/Comb_Seg/DB2_s' + str(j) + 'Seg.h5', 'w')
