...
if __name__ == '__main__':
    for j in range(1,2):
        df = pd.read_hdf(dir+'/data/raw/DB2_s' + str(j) + 'raw1.h5', 'df')
        data = np.array(df)
        channel = 12
        index = []
        tempemg = []
    ...
    # 存储为h5文件
        df_down.to_hdf(dir+'/data/down/DB2_s' + str(j) + 'down1.h5', format='table', key='df', mode='w', complevel=9, complib='blosc')
        print()
